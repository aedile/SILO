 ...
 * script keys: coin start fire1 fire2 fire3 tx ty (trackball counts per frame, signed)
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static void write_ppm(const char *path, const uint8_t *fb, const uint16_t *pal)
{
    ...
    if (!f) return;
    fprintf(f, "P6\n%d %d\n255\n", MC_FB_W, MC_FB_H);
    for (int y = 0; y < MC_FB_H; y++) {
        for (int x = 0; x < MC_FB_W; x++) {
            uint16_t c = pal[fb[y * MC_FB_W + x] & 7];
...
int main(int argc, char **argv)
{
    ...
    int r10 = 0x81, r8 = 0x73;   /* factory: English, 6 cities, bonus city at 10000 */
    ...
    mc_roms_t roms = { mc_rom, mc_writeprom };
    mc_init(&roms);
    mc_set_dips((uint8_t)r10, (uint8_t)r8);
    mc_input_t *in = mc_input();
    ...
    static uint8_t fb[MC_FB_W * MC_FB_H];
    static int16_t abuf[4096];
    uint16_t pal[MC_PALETTE_SIZE];
    const double fps = 61.035;
    ...
    for (int f = 0; f < frames; f++) {
        ...
        for (int e = 0; e < nev; e++) {
            if (evs[e].t <= now && evs[e].t > now - 1.0 / fps) {
                ...
                else if (!strcmp(k, "fire1")) in->fire1 = v; else if (!strcmp(k, "fire2")) in->fire2 = v;
                else if (!strcmp(k, "fire3")) in->fire3 = v;
                else if (!strcmp(k, "tx")) in->track_x = (int8_t)v; else if (!strcmp(k, "ty")) in->track_y = (int8_t)v;
            }
        }
        mc_run_frame();
        if (wav) {
            audio_acc += rate / fps; int n = (int)audio_acc; audio_acc -= n;
            mc_render_audio(abuf, n, rate); fwrite(abuf, 2, n, wav); wav_samples += n;
        }
        if (now >= next_save) {
            char path[512]; snprintf(path, sizeof(path), "%s/frame_%03d.ppm", outdir, saved);
            mc_render(fb); mc_palette(pal); write_ppm(path, fb, pal); saved++; next_save += every;
        }
        if ((f % (int)fps) == (int)fps - 1) printf("t=%ds pc=%04X irqs=%u\n", (int)(now + 1), mc_pc(), mc_irq_count());
    }
    if (wav) {
